...
var buildData = days.map(day => ({ date: day, pass: Math.random() > 0.1 }));
var compilerTestData = days.map(day => ({ date: day, pass: Math.random() > 0.1 }));
var servicesTestData = days.map(day => ({ date: day, pass: Math.random() > 0.1 }));
...
var parseData = days.map(day => ({ x: day, y: parseRandom() }));
var typecheckData = days.map(day => ({ x: day, y: typecheckRandom() }));
var emitData = days.map(day => ({ x: day, y: emitRandom() }));
